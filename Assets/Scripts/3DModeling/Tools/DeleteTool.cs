...
public class DeleteTool : OperationTool
{
    ...
    {
        if (ModelEditingPanel.Instance.GetEditMode() == EditMode.Pivot)
        {
            return false;
        }
        else
    ...
    {
        if (ModelEditingPanel.Instance.GetEditMode() == EditMode.Object && SelectionManager.Instance.GetSelectedModels().Count != 0)
        {
            List<ModelData> selectedObjects = SelectionManager.Instance.GetSelectedModels();
            foreach(var model in selectedObjects)
            {
                model.DeleteModel();
            }
        }
        else if (ModelEditingPanel.Instance.GetEditMode() == EditMode.Vertex || ModelEditingPanel.Instance.GetEditMode() == EditMode.Edge || ModelEditingPanel.Instance.GetEditMode() == EditMode.Face)
        {
            //List<ModelData> selectedObjects = SelectionManager.Instance.GetSelectedModels();
            //selectedObjects.ForEach(model => DeleteSelectedElements(model,));
        }
